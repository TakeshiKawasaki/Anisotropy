...
int calc_force(double* x, double* y, double L, int Np, double* a, double* kx, double* ky,double *knx, double* kny,int (*list)[Pm],double *theta,double chi,double chi_pr,double mu,double eta) {
  int i, j, k;
  // *avU = 0.0;
 ...
 for (i = 0; i < Np; i++)
      ...
      for (j = 1; j <=list[i][0]; j++)
	{
      ...
      aij_pr3 = aij_pr*aij_pr*aij_pr;
        ...
        knx[i]= diU*dx + dcU*cos(theta[list[i][j]]);
        knx[list[i][j]]=djU*dx + dcU*cos(theta[i]);

        kny[i]=diU*dy + dcU*sin(theta[list[i][j]]);
        kny[list[i][j]]=djU*dy + dcU*sin(theta[i]);
...
int eq_motion(double* x, double* y,double* theta, double* vx, double* vy,double* nx,double* ny,double* nx_pr,double* ny_pr, double *omega, double dt, double* kx, double* ky, double*knx, double* kny, int Np, double* avK, double Th) {
  ...
  *avK=0.0;
  for (k = 0; k < Np; k++) {
    ...
    omega[k] = sqrt(nx_pr[k] * nx_pr[k] + ny_pr[k] * ny_pr[k]);
...
int main(int argc, char *argv[])
{
  ...
  Th=0.3;
  for(;;){ // infinite loop
    calc_force(x, y, L, Np, a, kx, ky,knx,kny,list,theta,chi,chi_pr,mu,eta);    
