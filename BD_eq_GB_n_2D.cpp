...
int output(double *x,double *y, double* nx,double x_corr,double y_corr,double L,double *theta,double *r1,int Np){
  int i;
  static int count_file=1;
  double x1[Npm],y1[Npm];
  char filename[128];   
  sprintf(filename,"coord_%.d_0.3_0.95.dat",count_file);
  ofstream file;
  file.open(filename);
  for(i=0;i<Np;i++){
    x1[i]=x[i]-x_corr;
...
void ini_hex(double *x,double *y,double* nx, double* ny,double *theta,double *a,double L,int Np){
  ...
  for(j=0;j<num_y;j++){
    for(i=0;i<num_x;i++){
      ...
      nx[i+num_x*j] = 1/sqrt(2);
      ny[i+num_x*j] = 1/sqrt(2);
...
int calc_force(double* x, double* y, double* nx, double* ny, double L, int Np, double* a, double* kx, double* ky,double *knx, double* kny,int (*list)[Pm],double *theta,double chi,double chi_pr,double mu,double eta) {
  int i, j, k;
  // *avU = 0.0;
 ...
 for (i = 0; i < Np; i++)
      ...
      for (j = 1; j <=list[i][0]; j++)
	{
      ...
      aij_pr3 = aij_pr*aij_pr*aij_pr;
        ...
        dcU = 4.*pow(e1,eta)*pow(e2,mu)*(((A*mu*chi_pr*chi_pr)/2./e2/r2)*(R1p_pr*R1p_pr-R1n_pr*R1n_pr)-((aij_pr3*chi*chi*B)/4./aij/aij/r2)*(R1p*R1p-R1n*R1n)+A*eta*chi*chi*cij*e1*e1);//analytical calculation of the 1'st derivative / ri*ni
...
int eq_motion(double* x, double* y,double* theta, double* vx, double* vy,double* nx,double* ny,double* nx_pr,double* ny_pr, double *omega, double dt, double* kx, double* ky, double*knx, double* kny, int Np, double* avK, double Th) {
  double zeta;
  double n[Np];
  double nxp[Np];
  double nyp[Np];
  ...
  *avK=0.0;
  for (k = 0; k < Np; k++) {
    ...
    nx_pr[k] += -nx_pr[k] * zeta * dt + knx[k] * dt + sqrt(2. * zeta * Th * dt) * gaussian_rand();
    ny_pr[k] += -ny_pr[k] * zeta * dt + kny[k] * dt + sqrt(2. * zeta * Th * dt) * gaussian_rand();
    ...
    omega[k] = sqrt(nx_pr[k] * nx_pr[k] + ny_pr[k] * ny_pr[k])/n[k];
...
int main(int argc, char *argv[])
{
  ...
  double* x, * y,* x0, * y0, * vx, * vy, * a, * kx, * ky,* knx,* kny,* nx,* ny,* nx_pr, * ny_pr,*x_update,*y_update,*theta,*omega;
  ...
  Th=0.3;
  for(;;){ // infinite loop
    calc_force(x, y, nx,ny, L, Np, a, kx, ky,knx,kny,list,theta,chi,chi_pr,mu,eta);    
    eq_motion(x, y, theta, vx, vy, nx, ny, nx_pr, ny_pr, omega, dt, kx, ky,knx, kny, Np, &avK0,Th);
    com_correction(x,y,&x_corr,&y_corr,Np, L);
    p_bound(x, y, Np, L);
    timer += dt;
     cout <<"disp"<< disp_ave <<"x="<<x[1]<<" "<<avK0<< endl;      
    if(disp_ave>disp_th*disp_th){ // the condition for changing temp
      output(x,y,nx,x_corr,y_corr,L,theta,a,Np); // does it mean every output has different temp??
	break;
    }
