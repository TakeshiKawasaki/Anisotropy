...
int output(double *x,double *y, double* nx,double* ny,double x_corr,double y_corr,double L,double *theta,double *r1,int Np){
  ...
  char filename[128];   
  sprintf(filename,"coord_0.3_0.3_rand_affine.dat");
  ofstream file;
  file.open(filename);
  for(i=0;i<Np;i++){
    x1[i]=x[i]-x_corr;
...
void affine_coord(double *x,double *y, double phi, int Np) {
  for(int k = 0; k < Np; k++){
    x[k] = sqrt(phi/(phi + 0.001*exp(-10.*phi)))*x[k];
    y[k] = sqrt(phi/(phi + 0.001*exp(-10.*phi)))*y[k];
  }
}
...
int main(int argc, char *argv[])
{
 ...
 // the condition for changing temp
    disp_ave = 0.0;
    for(;;){
    calc_force(x, y, nx,ny, L, Np, a, kx, ky,knx,kny,kth,list,theta,chi,chi_pr,mu,eta);    
    eq_motion(x, y, theta, vx, vy, nx, ny, nx_pr, ny_pr, omega, dt, kx, ky,knx, kny,kth, Np, &avK0,temp);
    com_correction(x,y,&x_corr,&y_corr,Np, L);
    p_bound(x, y, Np, L);
     cout <<"disp="<< disp_ave <<"x="<<x[1]<<" "<< "temp=" << avK0 << "L=" << L <<" "<< "M="<<M << " "<< "phi = " << phi <<endl;
