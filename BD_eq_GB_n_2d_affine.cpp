...
int output(double *x,double *y, double* nx,double* ny,double x_corr,double y_corr,double L,double *theta,double *r1,int Np){
  ...
  char filename[128];   
  sprintf(filename,"coord_0.3_%.d_rand_affine.dat",count_file);
  ofstream file;
  file.open(filename);
  for(i=0;i<Np;i++){
    x1[i]=x[i]-x_corr;
...
void affine_coord(double *x,double *y, double phi, int Np) {
  for(int k = 0; k < Np; k++){
    x[k] = (sqrt((phi-0.1)/phi))*x[k];
    y[k] = (sqrt((phi-0.1)/phi))*y[k];
  }
}
...
int main(int argc, char *argv[])
{
  ...
  char filename[128];
  avU0=0.0;
   for(phi = 0.1; phi < 1.1; phi += 0.1){
    L = (r1+r2)/4.0*sqrt(pow(2.,1./3.)*pi*kappa*Np/phi);
    M=(int)(L/RCHK);
  cout << "L=" << L <<" "<< "M="<<M << " "<< "phi = " << phi <<endl;
  // ini_coord_square(x,y,theta,a,L,Np);
  if(count >= 1){ // Do this procedure unless the time we passes for the first time
  affine_coord(x, y, phi, Np);
  }
  ...
  for(;;){ // infinite loop
    ...
    p_bound(x, y, Np, L);
    timer += dt;
     cout <<"disp="<< disp_ave <<"x="<<x[1]<<" "<< "temp=" << avK0 <<endl;      
    if(disp_ave>disp_th*disp_th){ // the condition for changing temp
      output(x,y,nx,ny,x_corr,y_corr,L,theta,a,Np); // does it mean every output has different temp
      char filename[128];   
      sprintf(filename,"phi_L_check.dat");
      ofstream file;
      file.open(filename);
      file << "phi = " << phi << "L = " << L ;
      file.close();
