...
int  set_map(double *x,double *y,int (*map)[Nn],double *a,double L,int Np,double chi,double *theta,double kappa,double* nx,double* ny)
{
  int i,j;
  int n;
  double r2,theta_i,theta_j,l;
  double as,al;
  double dx,dy;
  double Ai,Aj,A;
  ...
  for(i=0;i<Np;i++)
	...
	if(i!=j)
      ...
      theta_i = atan(ny[i]/(nx[i]+0.0000001))-atan(dy/(dx+0.0000001));
      theta_j = atan(ny[i]/(nx[i]+0.0000001))-atan(dy/(dx+0.0000001));

      l =  (kappa*1.44/4.)*(1./sqrt((as/2)*(as/2)*sin(theta_j)*sin(theta_j)+(al/2)*(al/2)*cos(theta_j)*cos(theta_j))) + (kappa*1.44/4.)*(1./sqrt((as/2)*(as/2)*sin(theta_i)*sin(theta_i)+(al/2)*(al/2)*cos(theta_i)*cos(theta_i))); // 
	   // l = 1.2*kappa;
   //  cout << l << endl;
	    if(r2 < l*l)   // aij*pow((1.+A),1./3.)) is rij when particles tangent each other so it was expanded a lil bit to catch nn particles
	      {
...
int calc_order(double *x,double *y,double *theta,int Np, double L,double *phi6,double *phi4,double *phi2,double *s,double *avphi6,double *avphi4,double *avphi2,double *avs,double *a,double *gr,double *g6,double *g4,double *g2,double *gs,double chi,double *qx,double *qy,double *cq,double *avphiq,double kappa,double* nx, double* ny){
  ...
  double phiqx=0.0;
  double phiqy=0.0;
  set_map(x,y,map,a,L,Np,chi,theta,kappa,nx,ny);     
  ...
  for(i=0;i<Np;i++){
    ...
    for(j=1;j<=map[i][0];j++){
      ...
      if(dy>0.5*L) dy-=L;
      if(dy<-0.5*L) dy+=L;
      angle = atan(dx/(dy+0.000000001)); // arctangent // avoid o
    
      phi2c[i]+=cos(2.*angle)/(map[i][0]+0.000001); // element of bond locals summantion for l = 2
      phi2s[i]+=sin(2.*angle)/(map[i][0]+0.000001); // element of bond locals summantion for l = 2

      phi4c[i]+=cos(4.*angle)/(map[i][0]+0.000001); // element of bond locals summantion for l = 4
      phi4s[i]+=sin(4.*angle)/(map[i][0]+0.000001); // element of bond locals summantion for l = 4

      phi6c[i]+=cos(6.*angle)/(map[i][0]+0.000001); // element of bond locals summantion for l = 6
      phi6s[i]+=sin(6.*angle)/(map[i][0]+0.000001); // element of bond locals summantion for l = 6
      t=cos(theta[i])*cos(theta[map[i][j]])+sin(theta[i])*sin(theta[map[i][j]]);
      s[i]+=(2.*t*t-1); // local nematic order's term which is not divided by Nnn
    }
    
    phiqx+=cos(qx[1]*x[i]+qy[1]*y[i]);  // translation order
    phiqy+=sin(qx[1]*x[i]+qy[1]*y[i]);  // same
     
    s[i]/=(map[i][0]+0.000001); // local nematic order
    *avs+=s[i]/Np; // global nematic order
  ...
  *avphi2+=sqrt(av2s*av2s+av2c*av2c)/Np;
  ...
  for(i=0;i<Np;i++){
	...
	if(dy<-0.5*L) dy+=L;
	r=sqrt(dx*dx + dy*dy);
	gr[int(r/0.02)]+=1.0/(2.0*pi*r*0.02*Np*Np/L/L); // gr
	g6[int(r/0.02)]+=(phi6s[i]*phi6s[j]+phi6c[i]*phi6c[j])/(2.0*pi*r*0.02*Np*Np/L/L); // correlation function
	g4[int(r/0.02)]+=(phi4s[i]*phi4s[j]+phi4c[i]*phi4c[j])/(2.0*pi*r*0.02*Np*Np/L/L); // correlation function
	g2[int(r/0.02)]+=(phi2s[i]*phi2s[j]+phi2c[i]*phi2c[j])/(2.0*pi*r*0.02*Np*Np/L/L); // correlation function
	t=cos(theta[i])*cos(theta[j])+sin(theta[i])*sin(theta[j]);
	gs[int(r/0.02)]+=(2.0*t*t-1.0)/(2.0*pi*r*0.02*Np*Np/L/L); //correlation function
 
	for(k=1;k<=20;k++)
	  cq[int(r/0.02)]+=cos(qx[k]*dx+qy[k]*dy)/(20.0*2.0*pi*r*0.02*Np*Np/L/L); // += cos(|exp(iq*rij)|)
      }
     cout << "pass" << endl;
    }
    cout << "cant pass" << endl; // segmentation fault would happen here
  }
...
int output(double *x,double *y, double* nx,double* ny,double x_corr,double y_corr,double L,double *theta,double *r1,int Np){
  ...
  char filename[128];   
  sprintf(filename,"coord_paper_3.0_0.47_10000.dat");
  ofstream file;
  file.open(filename);
  for(i=0;i<Np;i++){
    x1[i]=x[i]-x_corr;
...
int calc_force(double* x, double* y, double* nx, double* ny, double L, int Np, double* a, double* kx, double* ky,double *knx, double* kny, double* kth,int (*list)[Pm],double *theta,double chi,double chi_pr,double mu,double eta,double* avU,double *avU2) {
  int i, j, k;
  // *avU = 0.0;
...
int main(int argc, char *argv[])
{
  ...
  int i,count=0,count0=0,count_num_update=0;

  double disp_max=0.0;
  double disp_ave=0.0;
  int Np = 10000;int count_th=10;
  ...
  double* x, * y,* x0, * y0, * vx, * vy, * a, * kx, * ky,* knx,* kny,*kth,* nx,* ny,* nx_pr, * ny_pr,*x_update,*y_update,*theta,*omega,*phi6,*phi4,*phi2,*s,*gr,*g6,*g4,*g2,*gs,*cq,*qx,*qy;;
  ...
  copy(x0,y0,x,y,Np,x_corr,y_corr);
   ...
   for(int count=0;count<1000;count++){
    calc_force(x, y, nx,ny, L, Np, a, kx, ky,knx,kny,kth,list,theta,chi,chi_pr,mu,eta,&dummy,&dummy);    
   ...
   cout <<"disp="<< disp_ave <<"x="<<x[1]<<" "<< "temp=" << avK0 << "L=" << L <<" "<< "M="<<M << " "<< "phi = " << phi << "count = " << count <<endl;
    ...
    com_correction(x,y,&x_corr,&y_corr, Np, L);
    p_bound(x, y, Np, L);
    if(avK0 < fabs(temp+0.01)){   // abandon non eq parts
	//stage gate
	if(count0==count_th){ // it can pass only when its 10th times
	  sq(x,y,qx,qy,L,Np);
	//  cout <<"sq OK Th="<<temp<<endl;
	}
	//production
	if(count0>=count_th){ // after 10th times
	  calc_order(x,y,theta,Np,L,phi6,phi4,phi2,s,&avphi6,&avphi4,&avphi2,&avs,a,gr,g6,g4,g2,gs,chi,qx,qy,cq,&avphiq,kappa,nx,ny);
	  calc_force(x, y,nx, ny, L, Np, a, kx, ky,knx,kny,kth,list,theta,chi,chi_pr,mu,eta,&avU,&avU2);  
	   cout <<count0 <<endl;
	}
	count0++;
	avK+=avK0;
   ...
   cout <<"disp="<< disp_ave <<"x="<<x[1]<<" "<< "temp=" << avK0 << "L=" << L <<" "<< "M="<<M << " "<< "phi = " << phi << " "<<"count2=" << count2<<endl;
    ...
    // if(disp_ave>disp_th2){ 
      ...
      file <<temp<<" "<<avK/count0<<" "<<avU/(count0-count_th)<<" "<<(avU2/(count0-count_th)-avU*avU/(count0-count_th)/(count0-count_th))/Th/Th<<" "<<avphi6/(count0-count_th)<<" "<<avphi4/(count0-count_th)<<" "<<avphi2/(count0-count_th)<<" "<<avs/(count0-count_th)<<" "<<avphiq/(count0-count_th)<<endl; // to get av
	cout <<"T="<< temp << endl;
      break;
      }
   ///////auto update////////////////////
