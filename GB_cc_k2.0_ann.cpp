...
int output(double *x,double *y,double *z,double* nx, double* ny,double* nz,double x_corr,double y_corr,double z_corr,double L,double *theta1,double *theta2,double *r1,int Np,double temp, double c,double kappa,double phi,double anc,char *argv[],double phi_x){
  int i;
...
int output2(double *x,double *y,double *z,double* nx, double* ny,double* nz,double x_corr,double y_corr,double z_corr,double L,double *theta1,double *theta2,double *r1,int Np,double temp, double c,double kappa,double phi,double anc,char *argv[],double time,double phi_x){
  int i;
  static int count_file=1;
  double x1[Npm],y1[Npm],z1[Npm];
  char filename[128];   
	  ...
	  {  
	    ...
	    //if(r2< cut*cut){    
	      ...
	      *p += -(1./3.)*(dx*fx_ij + dy*fy_ij + dz*fz_ij + c*(dx*fx_ij_c + dy*fy_ij_c + dz*fz_ij_c));
	      ...
	      U1=4.*(pow(e1,eta)*pow(e2,mu)*A +0.25);
	      *avU +=U1/Np;
	      *avU2+=U1*U1/Np;   
	    }
	  }
...
int main(int argc, char *argv[])
{
  ...
  sprintf(filename,"anc_k%.1f_na_pr_a_%.1fc%.1f_T%.1f_phif%.2f.txt",kappa,anc,c,temp,phi_x);
  ofstream file;
  file.open(filename);
  file <<"#"<<" "<<"phi"<<" "<<"rho"<<" "<<"temp"<<" "<<"avU"<<" "<<"cv"<<" "<<"p"<<" "<<"p/T"<<endl;

  sprintf(filename,"anc_ann_k%.1f_na_a_%.1fc%.1f_T%.1fphif%.2f.txt",kappa,anc,c,temp,phi_x);
  ofstream file2;
  file2.open(filename);
  ...
  for(phi_final = 0.01; phi_final <= phi_x; phi_final+= 0.01){
    ...
    for(;;){
      ...
      if(count2 == 100000){
	if(count_coord==10){ 
	 output(x,y,z,nx,ny,nz,x_corr,y_corr,z_corr,L,theta1,theta2,a,Np,temp,c,kappa,phi,anc,argv,phi_x); 
	  count_coord=0;}
...
for(;;){
      ...
      if(count3 ==  1000000){  //10000000  3000000 
	//if(count_coord==1){ 
        time+=100.;//1000.;
	  output2(x,y,z,nx,ny,nz,x_corr,y_corr,z_corr,L,theta1,theta2,a,Np,temp,c,kappa,phi,anc,argv,time,phi_x); 
	 // count_coord=0;}
