...
int output(double *x,double *y,double *z,double* nx, double* ny,double* nz,double x_corr,double y_corr,double z_corr,double L,double *theta1,double *theta2,double *r1,int Np,double temp, double c,double kappa,double phi,double anc,char *argv[]){
  int i;
  static int count_file=1;
  double x1[Npm],y1[Npm],z1[Npm];
  char filename[128];   
  sprintf(filename,"coord_anc_mix_phi%.2f_a%.1f_c%.1f_T%.1f_k%.1f_N%d.dat",phi,anc,c,temp,kappa,Np);
  ofstream file;
  file.open(filename);
  for(i=0;i<Np;i++){
    x1[i]=x[i];
...
int output2(double *x,double *y,double *z,double* nx, double* ny,double* nz,double x_corr,double y_corr,double z_corr,double L,double *theta1,double *theta2,double *r1,int Np,double temp, double c,double kappa,double phi,double anc,char *argv[],double time){
  int i;
  static int count_file=1;
  double x1[Npm],y1[Npm],z1[Npm];
  char filename[128];   
  sprintf(filename,"coord_t%.1f_anc_mix_phi%.2f_a%.1f_c%.1f_T%.1f_k%.1f_N%d.dat",time,phi,anc,c,temp,kappa,Np);
  ofstream file2;
  file2.open(filename);
  for(i=0;i<Np;i++){
    x1[i]=x[i];
...
int main(int argc, char *argv[])
{
  ...
  sprintf(filename,"anc_mix_na_a_%.1fc%.1f_T%.1f.txt",anc,c,temp);
  ofstream file;
  file.open(filename);
  file <<"#"<<" "<<"phi"<<" "<<"rho"<<" "<<"temp"<<" "<<"avU"<<" "<<"cv"<<" "<<"p"<<" "<<"p/T"<<endl;

  sprintf(filename,"anc_ann_mix_na_a_%.1fc%.1f_T%.1f.txt",anc,c,temp);
  ofstream file2;
  file2.open(filename);
  file2 <<"#"<<" "<<"phi"<<" "<<"rho"<<" "<<"temp"<<" "<<"avU"<<" "<<"cv"<<" "<<"p"<<" "<<"p/T"<<" "<<"t"<<endl;
  ...
  for(phi_final = 0.01; phi_final <= 0.8; phi_final+= 0.01){
    ...
    for(;;){
      ...
      if(count2 == 5000){
	//if(count_coord==1){ 
	 ///output(x,y,z,nx,ny,nz,x_corr,y_corr,z_corr,L,theta1,theta2,a,Np,temp,c,kappa,phi,anc,argv); 
	 // count_coord=0;}
...
for(;;){
      ...
      avU+=avU0;
      p += p0;
      //cout <<"disp="<< disp_ave <<"x="<<x[1]<<" "<< "temp=" << avK0 << "L=" << L <<" "<< "M="<<M << " "<< "phi = " << phi << " "<<"count3=" << count3<<endl;
      if(count3 == 10000000){
	//if(count_coord==1){ 
        time+=1000.;
	  output2(x,y,z,nx,ny,nz,x_corr,y_corr,z_corr,L,theta1,theta2,a,Np,temp,c,kappa,phi,anc,argv,time); 
	 // count_coord=0;}
	file2 <<phi<<" "<< rho << " " <<avK/count3<<" "<<avU/count3<<" "<<(avU2/count3-avU*avU/count3/count3)/temp/temp<<" "<<avK/count3 + p/count3 <<" " <<(avK/count3 + p/count3)/(avK/count3)<<" " << time <<" "<<temp<<endl; // to get av
	count3=0;
     avK=0.0,avK0=0.0, p0=0.0,p=0.0,avU=0.0,avU2=0.0;
     temp-=0.05;
     k++;}
