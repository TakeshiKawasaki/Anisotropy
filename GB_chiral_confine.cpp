...
int output(double *x,double *y,double *z,double* nx, double* ny,double* nz,double x_corr,double y_corr,double z_corr,double L,double *theta1,double *theta2,double *r1,int Np){
 int i;
  ...
  char filename[128];   
  sprintf(filename,"coord_c2_chiral_confine_GBwall_bete_T0.1_Np10000_phi0.%d.dat",count_file);
  ofstream file;
  file.open(filename);
  for(i=0;i<Np;i++){
    x1[i]=x[i]-x_corr;
...
int main(int argc, char *argv[])
{
 ...
 initial(nx,ny,nz,Np);
  ...
  sprintf(filename,"phi_temp_c2_T0.1_chiral.txt");
  ofstream file;
  file.open(filename);
...
for(;;){ // infinite loop
   ...
   for(int count=0;count<5000;count++){
    calc_force(x, y, z, L, nx, ny, nz, Np, a, kx, ky, kz, knx,kny,knz,kw,kx_w, ky_w, kz_w, knx_w,kny_w,knz_w,list,theta1,theta2,chi,chi_pr,mu,eta,kappa,kx_c, ky_c, kz_c, knx_c,kny_c,knz_c,c);   
