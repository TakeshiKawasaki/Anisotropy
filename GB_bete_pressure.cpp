...
int main(int argc, char *argv[])
{
  ...
  double t,avU=0.0,avU2=0.0,avU0=0.0,avK=0.0,avK0=0.0,p=0.0,dummy;
...
for(;;){ // infinite loop
   ...
   for(int count=0;count<10000;count++){//1000
    calc_force(x, y, z, L, nx, ny, nz, Np, a, kx, ky, kz, knx,kny,knz,list,theta1,theta2,chi,chi_pr,mu,eta,&p);   
    eq_motion(x, y,z, theta1, theta2,vx, vy, vz, nx, ny, nz, nx_pr, ny_pr, nz_pr, omega, dt, kx, ky, kz, knx, kny,knz, Np, &avK0,temp,&p);
...
//cout <<"disp= "<< disp_ave <<" " << "x= "<<" " << x[1]<<" "<< "temp=" << avK0 << "L=" << L <<" "<< "M="<<M << " "<< "phi = " << phi << "count = " << count <<endl;
 ...
 //cout <<"disp="<< disp_ave <<"x="<<x[1]<<" "<< "temp=" << avK0 << "L=" << L <<" "<< "M="<<M << " "<< "phi = " << phi << " "<<"count2=" << count2<<endl;
    ...
    // if(disp_ave>disp_th2){ 
     ...
     file <<temp<<" "<<avK/count2<<" "<<avU/count2<<" "<<(avU2/count2-avU*avU/count2/count2)/temp/temp<<" "<<p/count2<<endl; // to get av
      break;
      }
   ///////auto update////////////////////
    calc_disp(&disp_max,&disp_ave,x,y,z,x_update,y_update,z_update,x0,y0,z0,Np,L,x_corr,y_corr,z_corr);
