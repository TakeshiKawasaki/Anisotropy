...
int calc_force(double* x, double* y, double* z, double L, double* nx, double* ny, double* nz,int Np, double* a, double* kx, double* ky,double* kz,double *knx,double *kny,double *knz,int (*list)[Pm],double *theta1,double *theta2,double chi,double chi_pr,double mu,double eta,double* p) {
  int i, j, k;
  // *avU = 0.0;
...
for(phi_final = 0.1; phi_final <= 1.0; phi_final+= 0.1){
   // avU=0.0,avU2=0.0,disp_ave=0.0;avK=0.0;


for(;;){ // infinite loop

   for(int count=0;count<10000;count++){//1000
    calc_force(x, y, z, L, nx, ny, nz, Np, a, kx, ky, kz, knx,kny,knz,list,theta1,theta2,chi,chi_pr,mu,eta,&dummy);   
...
//cout <<"disp= "<< disp_ave <<" " << "x= "<<" " << x[1]<<" "<< "temp=" << avK0 << "L=" << L <<" "<< "M="<<M << " "<< "phi = " << phi << "count = " << count <<endl;
    ...
    int count2 = 0;
    for(;;){
    calc_force(x, y, z, L, nx, ny, nz, Np, a, kx, ky, kz, knx,kny,knz,list,theta1,theta2,chi,chi_pr,mu,eta,&p0);  
 ...
 //cout <<"disp="<< disp_ave <<"x="<<x[1]<<" "<< "temp=" << avK0 << "L=" << L <<" "<< "M="<<M << " "<< "phi = " << phi << " "<<"count2=" << count2<<endl;
    ...
    // if(disp_ave>disp_th2){ 
     ...
     file <<phi<<" "<<avK/count2<<" "<<avU/count2<<" "<<(avU2/count2-avU*avU/count2/count2)/temp/temp<<" "<<p/count2<<endl; // to get av
      break;
      }
   ///////auto update////////////////////
    calc_disp(&disp_max,&disp_ave,x,y,z,x_update,y_update,z_update,x0,y0,z0,Np,L,x_corr,y_corr,z_corr);
