...
int output_mid(double *x,double *y,double* z,double* nx, double* ny,double* nz,double x_corr,double y_corr,double z_corr,double L,double *theta1,double *theta2,double *r1,int Np,double t,double dt,char *argv[]){
  int i;
  static int count_file=1;
  double x1[Npm],y1[Npm],z1[Npm];
  char filename[128];   
  sprintf(filename,"%s/coord_longcal_T0.5_bete_cut5.9_Np10000_phi%d.dat",argv[1],count_file);
  ofstream file;
  file.open(filename);
  for(i=0;i<Np;i++){
    x1[i]=x[i]-x_corr;
...
void ini_fcc(double *x,double *y,double*z,double* nx,double* ny, double* nz, double* theta1, double* theta2, double *a,double L,int Np,double* r1){
...
int calc_force(double* x, double* y, double* z, double L, double* nx, double* ny, double* nz,int Np, double* a, double* kx, double* ky,double* kz,double *knx,double *kny,double *knz,int (*list)[Pm],double *theta1,double *theta2,double chi,double chi_pr,double mu,double eta,double* p,double* avU,double *avU2) {
  int i, j, k;
  // *avU = 0.0;
 ...
 for (i = 0; i < Np; i++)
      ...
      for (j = 1; j <=list[i][0]; j++)
	{
	  ...
	  // cout<<theta[list[i][j]]<<endl;
          ...
          r3 = r2*r;
          r4 = r2*r2;
	  if(r2< 5.9*5.9){  // the force by potential energy would work if the distance is smaller than 3*(charcteristic length) maybe i better put it just after calculating r??
...
int eq_motion(double* x, double* y,double* z,double* theta1,double* theta2, double* vx, double* vy, double* vz,double* nx,double* ny,double* nz, double* nx_pr,double* ny_pr,double* nz_pr, double* omega, double dt, double* kx, double* ky, double* kz,double* knx,double* kny,double* knz, int Np, double* avK, double Th,double* p) {
  double zeta, zeta_r;
  double n;//[Np];
  double n_pr2;//[Np];
  double Rx;//[Np];
...
int main(int argc, char *argv[])
{
  ...
  double t,avU=0.0,avU2=0.0,avU0=0.0,avK=0.0,avK0=0.0,p=0.0,p0=0.0,dummy;
  ...
  double* x, * y, *z,* x0, * y0, *z0,* vx, * vy,*vz, * a, * kx, * ky,*kz, * knx,* kny,* knz, * nx,* ny,* nz, * nx_pr,* ny_pr,* nz_pr,*x_update,*y_update,*z_update, *theta1, *theta2 ,*omega,*r1;
  ...
  ini_fcc(x,y,z,nx,ny,nz,theta1,theta2,a,L,Np,r1);
...
for(;;){ // infinite loop
   ...
   for(int count=0;count<10000;count++){//1000
    calc_force(x, y, z, L, nx, ny, nz, Np, a, kx, ky, kz, knx,kny,knz,list,theta1,theta2,chi,chi_pr,mu,eta,&dummy,&dummy,&dummy);   
    eq_motion(x, y,z, theta1, theta2,vx, vy, vz, nx, ny, nz, nx_pr, ny_pr, nz_pr, omega, dt, kx, ky, kz, knx, kny,knz, Np, &avK0,temp,&dummy);
...
//cout <<"disp= "<< disp_ave <<" " << "x= "<<" " << x[1]<<" "<< "temp=" << avK0 << "L=" << L <<" "<< "M="<<M << " "<< "phi = " << phi << "count = " << count <<endl;
    ...
    affine_coord(x, y, z, phi, Np,dphi);  
     ...
     //phi += 0.001*exp(-phi);   
      ...
      M=(int)(L/RCHK); 
     // if(count_out == 200){
      //output_mid(x,y,z,nx,ny,nz,x_corr,y_corr,z_corr,L,theta1,theta2,r1,Np,t,dt,argv);
      //cout<< "output" << endl;
       //count_out=0;
      //}
    ...
    for(;;){
    p0=0.0;
    calc_force(x, y, z, L, nx, ny, nz, Np, a, kx, ky, kz, knx,kny,knz,list,theta1,theta2,chi,chi_pr,mu,eta,&p0,&avU,&avU2);  
    eq_motion(x, y,z, theta1, theta2,vx, vy, vz, nx, ny, nz, nx_pr, ny_pr, nz_pr, omega, dt, kx, ky, kz, knx, kny,knz, Np, &avK0,temp,&p0);
    com_correction(x,y,z,&x_corr,&y_corr,&z_corr, Np, L);
    p_bound(x, y, z, Np, L);
    avK+=avK0;
    p+=p0;
 //cout <<"disp="<< disp_ave <<"x="<<x[1]<<" "<< "temp=" << avK0 << "L=" << L <<" "<< "M="<<M << " "<< "phi = " << phi << " "<<"count2=" << count2<<endl;
     if(count2 >= 100000){//100000
    // if(disp_ave>disp_th2){ 
      output(x,y,z,nx,ny,nz,x_corr,y_corr,z_corr,L,theta1,theta2,r1,Np); 
     file <<phi<<" "<<avK/count2<<" "<<avU/count2<<" "<<(avU2/count2-avU*avU/count2/count2)/temp/temp<<" "<<avK/count2 + p/count2<<endl; // to get av
      break;
      }
   ///////auto update////////////////////
    calc_disp(&disp_max,&disp_ave,x,y,z,x_update,y_update,z_update,x0,y0,z0,Np,L,x_corr,y_corr,z_corr);
