...
int output(double *x,double *y,double *z,double* nx, double* ny,double* nz,double x_corr,double y_corr,double z_corr,double L,double *theta1,double *theta2,double *r1,int Np){
  ...
  char filename[128];   
  sprintf(filename,"coord_T0.5_phi1.0_bete_Np10000.dat");
  ofstream file;
  file.open(filename);
  for(i=0;i<Np;i++){
    x1[i]=x[i]-x_corr;
...
int output_mid(double *x,double *y,double* z,double* nx, double* ny,double* nz,double x_corr,double y_corr,double z_corr,double L,double *theta1,double *theta2,double *r1,int Np,double t,double dt,char *argv[]){
  ...
  double x1[Npm],y1[Npm],z1[Npm];
  char filename[128];   
  sprintf(filename,"%s/coord_mid_T0.5_bete_Np10000_phi%d.dat",argv[1],count_file);
  ofstream file;
  file.open(filename);
  for(i=0;i<Np;i++){
    x1[i]=x[i]-x_corr;
 ...
 for (i = 0; i < Np; i++)
      ...
      for (j = 1; j <=list[i][0]; j++)
	{
	  ...
	  // cout<<theta[list[i][j]]<<endl;
          ...
          r3 = r2*r;
          r4 = r2*r2;
	  if(r2< 9.*9.){  // the force by potential energy would work if the distance is smaller than 3*(charcteristic length) maybe i better put it just after calculating r??
...
int main(int argc, char *argv[])
{
  ...
  double temp = 0.5; // do  1.2 1.0 0.8 0.5 
  double Th;
  double phi=0.001;
  double dphi = 0.0005;
