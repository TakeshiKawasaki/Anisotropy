...
int calc_force_chiral(double* x, double* y, double* z, double L, double* nx, double* ny, double* nz,int Np, double* a, double* kx, double* ky,double* kz,double *knx,double *kny,double *knz,int (*list)[Pm],double *theta1,double *theta2,double chi,double chi_pr,double mu,double eta， double chiral) {
  ...
  double R2p,R2n,R1p,R1n,R2p_pr,R2n_pr,R1p_pr,R1n_pr;
  double t,drU,diU,djU,dcU,U,A,B,A1,A2,A6,A7,A8,fx_ij,fy_ij,fz_ij,U1;
  double e1,e2;
  double dx, dy, dz;
  double aij, aij_pr, aij3, aij_pr3;
  double cut;
 ...
 for (i = 0; i < Np; i++)
      ...
      for (j = 1; j <=list[i][0]; j++)
	{
	  ...
	  if(r2< 3.0*3.0){  // the force by potential energy would work if the distance is smaller than 3*(charcteristic length) maybe i better put it just after calculating r??
	    ...
	    rj = dx*nx[list[i][j]] + dy*ny[list[i][j]] + dz*nz[list[i][j]];
	    cij = nx[i]*nx[list[i][j]] + ny[i]*ny[list[i][j]] + nz[i]*nz[list[i][j]];
	    dij = (ny[i]*nz[list[i][j]]-nz[i]*ny[list[i][j]])*dx
           +(nz[i]*nx[list[i][j]]-nx[i]*nz[list[i][j]])*dy
           +(nx[i]*ny[list[i][j]]-ny[i]*nx[list[i][j]])*dz;
...
//        knz[list[i][j]] -= djU * (ny[list[i][j]]*ny[list[i][j]]*dz - ny[list[i][j]]*nz[list[i][j]]*dy - nx[list[i][j]]*nz[list[i][j]]*dx + nx[list[i][j]]*nx[list[i][j]]*dz) + dcU*(nz[i]*ny[list[i][j]]*ny[list[i][j]] - ny[i]*ny[list[i][j]]*nz[list[i][j]] - nx[i]*nx[list[i][j]]*nz[list[i][j]] + nz[i]*nx[list[i][j]]*nx[list[i][j]]);
        ...
        knx[i] -= diU*(dx - ri*nx[i]) + dcU*(nx[list[i][j]] - cij*nx[i]) + chiral*4.*dij*pow(e1,eta)*pow(e2,mu)*(1.-cij)*nx[i];
        knx[list[i][j]] -= djU*(dx - rj*nx[list[i][j]]) + dcU*(nx[i] - cij*nx[list[i][j]]) + chiral*4.*dij*pow(e1,eta)*pow(e2,mu)*(1.-cij)*nx[list[i][j]];

        kny[i] -= diU*(dy - ri*ny[i]) + dcU*(ny[list[i][j]] - cij*ny[i])+ chiral*4.*dij*pow(e1,eta)*pow(e2,mu)*(cij+1.)*ny[i];
        kny[list[i][j]] -= djU*(dy - rj*ny[list[i][j]]) + dcU*(ny[i] - cij*ny[list[i][j]]) +chiral*4.*dij*pow(e1,eta)*pow(e2,mu)*(1.-cij)*ny[list[i][j]];

 	knz[i] -= diU*(dz - ri*nz[i]) + dcU*(nz[list[i][j]] - cij*nz[i]) + chiral*4.*dij*pow(e1,eta)*pow(e2,mu)*(1.-cij)*nz[i];
        knz[list[i][j]] -= djU*(dz - rj*nz[list[i][j]]) + dcU*(nz[i] - cij*nz[list[i][j]]) +chiral*4.*dij*pow(e1,eta)*pow(e2,mu)*(1.-cij)*nz[list[i][j]];
