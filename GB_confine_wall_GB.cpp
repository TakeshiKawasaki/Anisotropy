...
int output(double *x,double *y,double *z,double* nx, double* ny,double* nz,double x_corr,double y_corr,double z_corr,double L,double *theta1,double *theta2,double *r1,int Np){
 int i;
  ...
  char filename[128];   
  sprintf(filename,"coord_confineGBwall_bete_T1.0_Np10000_phi0.%d.dat",count_file);
  ofstream file;
  file.open(filename);
  for(i=0;i<Np;i++){
    x1[i]=x[i]-x_corr;
...
int calc_force(double* x, double* y, double* z, double L, double* nx, double* ny, double* nz,int Np, double* a, double* kx, double* ky,double* kz,double *knx,double *kny,double *knz,double*kw,double* kx_w, double* ky_w,double* kz_w,double *knx_w,double *kny_w,double *knz_w,int (*list)[Pm],double *theta1,double *theta2,double chi,double chi_pr,double mu,double eta,double kappa) {
  ...
  double rw_low,r2w_low,r3w_low,r4w_low,riw_low,rjw_low,cijw_low,rjw_high,cijw_high;
 ...
 for (i = 0; i < Np; i++)
    {
	  ...
	  // cout<<theta[list[i][j]]<<endl;
	    ...
	    R2pw_low =(riw_low+rjw_low)*(riw_low+rjw_low)/(1+chi*cijw_low);
 	    R2nw_low =(riw_low-rjw_low)*(riw_low-rjw_low)/(1-chi*cijw_low);
 	    R1pw_low =(riw_low+rjw_low)/(1+chi*cijw_low);
 	    R1nw_low =(riw_low-rjw_low)/(1-chi*cijw_low);

 	    R2p_pr_w_low =(riw_low+rjw_low)*(riw_low+rjw_low)/(1+chi_pr*cijw_low);
 	    R2n_pr_w_low =(riw_low-rjw_low)*(riw_low-rjw_low)/(1-chi_pr*cijw_low);
 	    R1p_pr_w_low =(riw_low+rjw_low)/(1+chi_pr*cijw_low);
 	    R1n_pr_w_low =(riw_low-rjw_low)/(1-chi_pr*cijw_low);  

        aij_pr_w_low = aij_w/sqrt(1.-(chi/2./r2w_low)*(R2pw_low+R2nw_low));
        aij_pr3_w_low = aij_pr_w_low* aij_pr_w_low* aij_pr_w_low;

	    R2pw_high =(riw_high+rjw_high)*(riw_high+rjw_high)/(1+chi*cijw_low);
 	    R2nw_high =(riw_high-rjw_high)*(riw_high-rjw_high)/(1-chi*cijw_low);
 	    R1pw_high =(riw_high+rjw_high)/(1+chi*cijw_low);
 	    R1nw_high =(riw_high-rjw_high)/(1-chi*cijw_low);

 	    R2p_pr_w_high =(riw_high+rjw_high)*(riw_high+rjw_high)/(1+chi_pr*cijw_low);
 	    R2n_pr_w_high =(riw_high-rjw_high)*(riw_high-rjw_high)/(1-chi_pr*cijw_low);
 	    R1p_pr_w_high =(riw_high+rjw_high)/(1+chi_pr*cijw_low);
 	    R1n_pr_w_high =(riw_high-rjw_high)/(1-chi_pr*cijw_low);    
...
//  }
  ...
  *avK=0.0;
  for (k = 0; k < Np; k++) {
    ...
    vz[k] += -vz[k] * zeta * dt + kz[k] * dt + kz_w[k] * dt + kw[k] * dt + sqrt(2. * zeta * Th * dt) * gaussian_rand();
...
int main(int argc, char *argv[])
{
  ...
  double* x, * y, *z,* x0, * y0, *z0,* vx, * vy,*vz, * a, * kx, * ky,*kz, * knx,* kny,* knz, * kx_w, * ky_w,*kz_w, * knx_w,* kny_w,* knz_w, * nx,* ny,* nz, * nx_pr,* ny_pr,* nz_pr,*x_update,*y_update,*z_update, *theta1, *theta2 ,*omega,*r1,*kw;
...
for(;;){ // infinite loop
   ...
   for(int count=0;count<10000;count++){
    calc_force(x, y, z, L, nx, ny, nz, Np, a, kx, ky, kz, knx,kny,knz,kw,kx_w, ky_w, kz_w, knx_w,kny_w,knz_w,list,theta1,theta2,chi,chi_pr,mu,eta,kappa);   
