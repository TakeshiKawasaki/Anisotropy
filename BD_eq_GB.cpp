...
int  set_map(double *x,double *y,int (*map)[Nn],double *a,double L,int Np,double chi,double *theta) // should i change ?? but how ??
{
  int i,j;
  int n;
  double r2;
	...
	if(i!=j)
	  {
	    ...
	    if(r2< aij*aij*pow((1.+A),1./3.)*1.2*1.2)  // aij*pow((1.+A),1./3.)) is rij when particles tangent each other so it was expanded a lil bit to catch nn particles
	      {
...
int sq(double *x,double *y,double *qx,double *qy,double L,int Np){ // to decide the first reciprocal vector
  int i,j,nx,ny;
...
int output(double *x,double *y,double x_corr,double y_corr,double L,double *theta,double *r1,int Np,double t,double dt,double *phi6,double *phi4,double *phi2,double *s,char *argv[],double *gr,double *g6,double *g4,double *gs,double *cq,int count,double avs,int count_th){
  ...
  char filename[128];   
  sprintf(filename,"%d/coord_%.d.dat",100,count_file);
  ofstream file;
  file.open(filename);
  for(i=0;i<Np;i++){
    x1[i]=x[i]-x_corr;
  ...
  sprintf(filename,"%d/gr_%.d.dat",100,count_file);
  file.open(filename);
  for(i=1;i<1500;i++)
...
int output_t(double *x,double *y,double *theta,double avU, double avK,int Np,double t,double time_stamp,double x_corr,double y_corr,char *argv[])
{
  ...
  sprintf(filename,"%d/time_coord.dat",100);
  fp=fopen(filename,"a+");
  ...
  sprintf(filename2,"%d/time_energy.dat",100);
...
int main(int argc, char *argv[])
{
  ...
  double Th;
  double chi= 100;  // 
  double chi_pr = 10; // its not finalised, not sure if its right value 
  double eta = 1;
  double mu = 2;
  double timer;
