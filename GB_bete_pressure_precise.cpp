...
int output(double *x,double *y,double *z,double* nx, double* ny,double* nz,double x_corr,double y_corr,double z_corr,double L,double *theta1,double *theta2,double *r1,int Np){
 int i;
  ...
  char filename[128];   
  sprintf(filename,"coord_p_T1.5_phi0.%d_bete_cut5.9_Np10000.dat",count_file);
  ofstream file;
  file.open(filename);
  for(i=0;i<Np;i++){
    x1[i]=x[i]-x_corr;
...
// cout << "done cell list" << endl;
  ...
  copy(x0,y0,z0,x,y,z,Np,x_corr,y_corr,z_corr);
   ...
   sprintf(filename,"p_pressure_T1.5.txt");
  ofstream file;
  file.open(filename);
...
for(;;){ // infinite loop
   for(int count=0;count<5000;count++){//1000
...
//cout <<"disp= "<< disp_ave <<" " << "x= "<<" " << x[1]<<" "<< "temp=" << avK0 << "L=" << L <<" "<< "M="<<M << " "<< "phi = " << phi << "count = " << count <<endl;
    ...
    //cout <<"else" << endl;
    for(int count=0;count<count_max;count++){//1000
    p0=0.0;
    ...
    //cout << phi<<" "<< rho << " " << avK/count << " " << avK/count + p/count <<endl;
    }
    file <<phi<<" "<< rho << " " <<avK/count_max<<" "<<avU/count_max<<" "<<(avU2/count_max-avU*avU/count_max/count_max)/temp/temp<<" "<<avK/count_max + p/count_max <<" " << (avK/count_max + p/count_max)/(avK/count_max) <<endl; // to get av
    }
     avK=0.0,avK0=0.0, p0=0.0,p=0.0,avU=0.0,avU2=0.0;
    }
     if(phi >= phi_final){
