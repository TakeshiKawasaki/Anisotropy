...
int output(double *x,double *y,double *z,double* nx, double* ny,double* nz,double x_corr,double y_corr,double z_corr,double L,double *theta1,double *theta2,double *r1,int Np){
 int i;
  ...
  char filename[128];   
  sprintf(filename,"coord_a_T1.5_phi0.%d_quick_bete_Np10000.dat",count_file);
  ofstream file;
  file.open(filename);
  for(i=0;i<Np;i++){
    x1[i]=x[i]-x_corr;
...
int main(int argc, char *argv[])
{
  ...
  double* x, * y, *z,* x0, * y0, *z0,* vx, * vy,*vz, * a, * kx, * ky,*kz, * knx,* kny,* knz, * nx,* ny,* nz, * nx_pr,* ny_pr,* nz_pr,*x_update,*y_update,*z_update, *theta1, *theta2 ,*omega,*r1,*cut;
...
// cout << "done cell list" << endl;
  ...
  copy(x0,y0,z0,x,y,z,Np,x_corr,y_corr,z_corr);
   ...
   sprintf(filename,"a_pressure_T1.5.txt");
  ofstream file;
  file.open(filename);
