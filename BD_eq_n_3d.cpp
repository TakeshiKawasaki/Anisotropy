...
for (k = 0; k < Np; k++) {
  ...
  *avK=0.0;
  for (k = 0; k < Np; k++) {
    ...
    nx_pr[k] += -nx_pr[k] * zeta_r * dt + knx[k] * dt + ny[k] * Rz[k] - nz[k] * Ry[k] ;
    ny_pr[k] += -ny_pr[k] * zeta_r * dt + kny[k] * dt + nz[k] * Rx[k] - nx[k] * Rz[k] ;
    nz_pr[k] += -nz_pr[k] * zeta_r * dt + knz[k] * dt + nx[k] * Ry[k] - ny[k] * Rx[k] ;   
