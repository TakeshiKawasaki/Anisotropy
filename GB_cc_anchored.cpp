...
int output(double *x,double *y,double *z,double* nx, double* ny,double* nz,double x_corr,double y_corr,double z_corr,double L,double *theta1,double *theta2,double *r1,int Np){
 int i;
  ...
  char filename[128];   
  sprintf(filename,"coord_anc_na_c1.0_T0.1_phi0.%d_chiral_confine_GBwall_bete_Np10000.dat",count_file);
  ofstream file;
  file.open(filename);
  for(i=0;i<Np;i++){
    x1[i]=x[i]-x_corr;
...
void ini_hex(double *x,double *y,double*z, double* nx, double* ny,double*nz,double *theta,double *a,double L,int Np,double kappa){
  ...
  for(j=0;j<num_y;j++){
    for(i=0;i<num_x;i++){
      ...
      z[i+num_x*j] = kappa*pow(2.,1./6.)/2.;//kappa*pow(2.,1./6.)/2.;
 ...
 for (i = 0; i < Np; i++)
    {
          ...
          lw_low = z[i] + kappa*pow(2.,1./6.)/2.;
          lw_high = z[i] - Lz - kappa*pow(2.,1./6.)/2.;
...
// n of j is always (0,0,1)//  MIRR0R ANCHORING
        ...
        knx_w[i] = diU_w_low*riw_low*nx[i] + dcU_w_low*cijw*nx[i] + diU_w_high*riw_high*nx[i] + dcU_w_high*cijw*nx[i];
  
        kny_w[i] = diU_w_low*riw_low*ny[i] + dcU_w_low*cijw*ny[i] + diU_w_high*riw_high*ny[i] + dcU_w_high*cijw*ny[i];

	    knz_w[i] = - diU_w_low*(lw_low - riw_low*nz[i]) - dcU_w_low*(1.0 - cijw*nz[i]) - diU_w_high*(lw_high - riw_high*nz[i]) - dcU_w_high*(1.0 - cijw*nz[i]);
	...
	{ 
	  ...
	  // cout<<theta[list[i][j]]<<endl;
        ...
        if(r2< cut*cut){  // the force by potential energy would work if the distance is smaller than 3*(charcteristic length) maybe i better put it just after calculating r??
...
//	    kth[list[i][j]]-=djU*(cos(theta[list[i][j]])*dy-sin(theta[list[i][j]])*dx)+dcU*(cos(theta[list[i][j]])*sin(theta[i])-sin(theta[list[i][j]])*cos(theta[i]));
         ...
         *p += -(1./3./V)*(dx*fx_ij + dy*fy_ij + dz*fz_ij + c*(dx*fx_ij_c + dy*fy_ij_c + dz*fz_ij_c));
...
int main(int argc, char *argv[])
{
 ...
 // cout << "ini" << endl;
  ...
  sprintf(filename,"anchor_na_c1.0_T0.1_chiral.txt");
  ofstream file;
  file.open(filename);
...
for(;;){ // infinite loop
   for(int count=0;count<5000;count++){
    calc_force(x, y, z, L, nx, ny, nz, Np, a, kx, ky, kz, knx,kny,knz,kw,kx_w, ky_w, kz_w, knx_w,kny_w,knz_w,list,theta1,theta2,chi,chi_pr,mu,eta,kappa,kx_c, ky_c, kz_c, knx_c,kny_c,knz_c,c,&dummy,&dummy,&dummy);   
 ...
 //cout <<"disp= "<< disp_ave <<" " << "x= "<<" " << x[1]<<" "<< "temp=" << avK0 << "L=" << L <<" "<< "M="<<M << " "<< "phi = " << phi << "count = " << count <<endl;
   ...
   int count2 = 0;
    ...
    for(;;){
    p0=0.0;
    calc_force(x, y, z, L, nx, ny, nz, Np, a, kx, ky, kz, knx,kny,knz,kw,kx_w, ky_w, kz_w, knx_w,kny_w,knz_w,list,theta1,theta2,chi,chi_pr,mu,eta,kappa,kx_c, ky_c, kz_c, knx_c,kny_c,knz_c,c,&p0,&avU,&avU2);   
    eq_motion(x, y,z, theta1, theta2,vx, vy, vz, nx, ny, nz, nx_pr, ny_pr, nz_pr, omega, dt, kx, ky, kz, knx, kny,knz,kw,kx_w, ky_w, kz_w, knx_w,kny_w,knz_w, Np, &avK0,temp,kx_c, ky_c, kz_c, knx_c,kny_c,knz_c,c);
    p_bound(x, y, z, Np, L);
    avK+=avK0;
    p += p0;
